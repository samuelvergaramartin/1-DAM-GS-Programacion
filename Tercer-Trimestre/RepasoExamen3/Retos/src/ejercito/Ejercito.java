...
public class Ejercito {
    ...
    public Personaje obtenerPersonajePorPosicionTablero(int posTableroX, int posTableroY) {
        Personaje personaje = null;
        int i = 0;

        while (i < EJERCITO.size() && personaje == null) {
            if(EJERCITO.get(i).getPosTableroX() == posTableroX && EJERCITO.get(i).getPosTableroY() == posTableroY) personaje = EJERCITO.get(i);
            i++;
        }
    ...
    private boolean posicionOcupada(int posX, int posY) {
        ...
        while (i < EJERCITO.size() && !resultado) {
            if(EJERCITO.get(i).getPosTableroX() == posX && EJERCITO.get(i).getPosTableroY() == posY) resultado = true;
            i++;
        }
